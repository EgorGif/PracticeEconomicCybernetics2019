...
public class FreeTimeClient {
    ...
    public static void main(String[] args) {
        try {
            ...
            Mud.RemoteFreeTimeServer server = (Mud.RemoteFreeTimeServer) Naming.lookup("rmi://" + hostname + ":" + FreeTimeServer.PORT + "/" + username);
...
//              currentPerson.setInMeet(cmd);
                switch (cmd) {
                    case "s": showFreeTime(server, currentPersonName); break;
                    case "u": updateFreeTime(server, currentPersonName); break;
                    case "m": makeAppointment(server, currentPersonName); break;
                    case "help":
    ...
    private static void checkMail(Mud.RemoteFreeTimeServer server, String personName) throws RemoteException {
        ...
        if (!person.getLetters().isEmpty()) {
            ...
            for (String letter : person.getLetters()) System.out.println(letter);
            System.out.flush();
            person.cleanLetters();
        }
    }
    private static void checkOffers(Mud.RemoteFreeTimeServer server, String personName) throws RemoteException, InterruptedException {
        ...
        synchronized (offers) {
            for (MeetingOffer offer : offers) {
                ...
                String answer = getLine("Do you want to meet \"" + offer.getFromWho() + "\" on Day "
                        + offer.getTime().getKey() + " at " + offer.getTime().getValue() + ":00?\n" + "y : yes\n" + "n : no\n" + "2 >> ");
                ...
                switch (answer) {
                    case "y":
                        ...
                        toSecondPerson = ">> You are meeting with \"" + personName + "\" on Day " +
                                offer.getTime().getKey() + " at " + offer.getTime().getValue() + ":00.\n";
                        break;
                    case "n":
                        toSecondPerson = ">> You aren't meeting with \"" + personName + "\" on Day " +
                                offer.getTime().getKey() + " at " + offer.getTime().getValue() + ":00.\n"; break;
        ...
        if (secondPerson != null) {
            ...
            if (!mutualFreeTime.isBusy()) {
                ...
                for (Pair<Integer, Integer> time : mutualFreeTime.getFreeTimeList()) {
                    String answer = getLine("Do you want to meet \"" + secondPersonName + "\" on Day " +
                            time.getKey() + " at " + time.getValue() + ":00?\n" + "y : yes\n" + "n : no\n" + ">> ");
                    switch (answer) {
                        case "y":
                ...
                if (!done) System.out.println(">> You and \"" + secondPersonName + "\" don't have no more opportunity to meet\n");
            } else System.out.println(">> You and \"" + secondPersonName + "\" don't have opportunity to meet\n");
        } else System.out.println(">> No user with this name was found.\n");
        System.out.flush();
    }
        ...
        do {
            ...
            switch (cmd) {
                case "r": {
                    ...
                    } else System.out.println(">> Name \"" + name + "\" is already used.");
                    break;
                }
                case "l": {
                    currentPerson = server.getPerson(name);
                    if (currentPerson != null) System.out.println(">> You are logged in as \"" + currentPerson.getName() + "\".");
                    else System.out.println(">> No user with this name was found.");
                    break;
                }
                default: {
...
//      System.out.flush();
    ...
    private static void updateFreeTime(Mud.RemoteFreeTimeServer server, String name) throws IOException {
        ...
        while (!done && (line = in.readLine()) != null) {
            ...
            for (int i = 2; i < words.length; i++) hours.add(Integer.parseInt(words[i]));
            try {
                switch (words[0]) {
                    case "-a": add.add(Integer.parseInt(words[1]), hours); break;
                    case "-r": remove.add(Integer.parseInt(words[1]), hours); break;
                    case "exit":
                    case "q":
                    case "quit": done = true; break;
                    default: System.out.println("Incorrect key \"" + words[0] + "\"");
                }
            } catch (Mud.IncorrectHour ih) { System.out.println("<error> Incorrect hour."); }
            catch (Mud.IncorrectDay id) { System.out.println("<error> Incorrect day."); }
        }
