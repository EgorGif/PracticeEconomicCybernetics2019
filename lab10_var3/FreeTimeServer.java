...
public class FreeTimeServer extends UnicastRemoteObject implements Mud.RemoteFreeTimeServer {
    ...
    public FreeTimeServer() throws RemoteException {/* For serialization only. Never call this constructor.*/}
    ...
    /*@Override
    public FreeTimePerson addPerson(String name) throws RemoteException {
        Optional<FreeTimePerson> optional = persons.stream().filter(p -> Objects.equals(p.getName(), name)).findFirst();
        if (!optional.isPresent()) {
            FreeTimePerson newPerson = new FreeTimePerson(name);
    ...
    public Mud.RemoteFreeTimePerson getPerson(String name) throws RemoteException {
        Optional<Mud.RemoteFreeTimePerson> optional = persons.stream().filter(p -> {
            try { return Objects.equals(p.getName(), name); }
            catch (RemoteException e) { System.err.println(e.getMessage()); }
            return false;
        }).findFirst();
        System.out.println("Persons are:");
        for (Mud.RemoteFreeTimePerson person : persons) System.out.println(person.toString());
        System.out.println("\n");
    ...
    public boolean addPerson(Mud.RemoteFreeTimePerson current) throws RemoteException {
        Optional<Mud.RemoteFreeTimePerson> optional = persons.stream().filter(p -> {
            try { return Objects.equals(p.getName(), current.getName()); }
            catch (RemoteException e) { System.err.println(e.getMessage()); }
            return false;
        }).findFirst();
        if (!optional.isPresent()) {
